...
export default function Playlists() {
  ...
  const [playlistItems, setPlaylistItems] = useState<IPlaylistItems[]>(
    getStorage("playlistItems")
  );
  const [theme] = useContext(ThemeContext);
  const [edit, setEdit] = useState(false);
  const [currentAba, setCurrentAba] = useState(
    playlists && playlists.length > 0 ? playlists[0].id : 0
  );
  ...
  const handlePlaylists = (id: number) => {
    playlists.map(
      (playlist: IPlaylists) => (playlist.active = playlist.id === id)
    );
  ...
  const removePlaylist = (id: number, index: number) => {
    ...
    if (playlistItems)
      modifyPlaylistItems(id, "").map((pI) =>
        updateResearchedIconPLaylist(pI.id, colorTag[1])
      );
  };

  const modifyPlaylistItems = (id: number, order: string) => {
    return order === "continue"
      ? playlistItems.filter((p) => id !== p.playlistId)
      : playlistItems.filter((p) => id == p.playlistId);
  };
    ...
    <>
      ...
      >
                ...
                return (
                  <>
                    <div
                      ...
                      onBlur={(e) => {
                        editPlaylistName(index, e.target.innerText);
                      }}
                    ...
                    >
                      ...
                      <span data-tip data-for={playlist.id}>
                        {playlist.name.substring(0, 35)}
                        {playlist.name.length > 35 ? "..." : ""}
                      </span>
                      <ReactTooltip
                        className={"tooltip"}
                        type={"info"}
        ...
        <div className="section">
          {currentAba && currentAba !== 0 ? (
            playlistItems
              .filter((video) => currentAba === video.playlistId)
              .slice(paagination[0], paagination[1])
              .map((video) => {
                return (
        ...
        {playlists && playlists.length > 0 ? (
          <div className="page-token">
            <div className="buttons">
              <button
                onClick={() =>
                  setPagination([paagination[0] - 6, paagination[1] - 6])
                }

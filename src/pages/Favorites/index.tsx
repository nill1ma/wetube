...
export default function Favorites() {
  ...
  const reAdjustResearchedStorage = (id: any) => {
    ...
    const lastest = lastestResearched.findIndex(
      (lastest: Videos) => lastest.id === id
    );
    ...
    <VideosContainer background={theme.section} color={theme.font}>
      ...
      <SingleVideoArea>
        {favoriteList() && favoriteList().length > 0 ? (
            ...
            return (
              <article key={video.id} className="video-box">
                <VideoBox
                  main={false}
                  video={video}
                  actions={[{ function: removeFavorite, icon: faTrashAlt }]}
                />
              </article>
            );
          })
          ...
          <EmpityMessage
            message={"You don`t have any videos in your favorite list"}
          />
        )}
      </SingleVideoArea>
